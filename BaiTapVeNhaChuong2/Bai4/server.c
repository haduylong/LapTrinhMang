...
/*
4. Viết chương trình sv_server, nhận dữ liệu từ
sv_client, in ra màn hình và đồng thời ghi vào file sv_log.
Ngoài dữ liệu được gửi từ client cần phải lưu lại địa chỉ
IP và thời gian client đã gửi. Tham số cổng và tên file log
được nhập từ tham số dòng lệnh.
*/

char* getStringTime(char *timerecv , struct tm date){
    sprintf(timerecv, "%d/%d/%d  %d:%d:%d", (date.tm_year + 1900), (date.tm_mon + 1), 
